...
enum DashTestData {
    ...
    static func masternode(proRegTxHash: Data, confirmedHash: Data = Data(), isValid: Bool = true) -> Masternode {
        Masternode(proRegTxHash: proRegTxHash, confirmedHash: confirmedHash, confirmedHashWithProRegTxHash: Data(), ipAddress: Data(), port: 0, pubKeyOperator: Data(), keyIDVoting: Data(), isValid: isValid)
    }
    ...
    static func coinbaseTransaction(transaction: FullTransaction = DashTestData.transaction, size: UInt32 = 0, merkleRootMNList: Data = Data()) -> CoinbaseTransaction {
        CoinbaseTransaction(transaction: transaction, coinbaseTransactionSize: Data(from: size), version: 0, height: 0, merkleRootMNList: merkleRootMNList)
    }
    ...
    {
        MasternodeListDiffMessage(baseBlockHash: baseBlockHash, blockHash: blockHash,
    ...
    static func transactionLockVote(txHash: Data = Data(), inputTxHash: Data = Data(), quorumModifierHash: Data = Data(), masternodeProTxHash: Data = Data(), vchMasternodeSignature: Data = Data(), hash: Data = Data()) -> TransactionLockVoteMessage {
        TransactionLockVoteMessage(txHash: txHash, outpoint: Outpoint(txHash: inputTxHash, vout: 0),
    ...
    static var transaction: FullTransaction {
        ...
        let outputs = [
            Output(withValue: 10_792_000, index: 0, lockingScript: Data(hex: "76a9141ec865abcb88cec71c484d4dadec3d7dc0271a7b88ac")!, type: .p2pkh, keyHash: Data()),
    ...
    static func unspentOutput(output: Output) -> UnspentOutput {
        UnspentOutput(output: output, publicKey: PublicKey(withAccount: 0, index: 0, external: false, hdPublicKeyData: Data()), transaction: Transaction(), blockHeight: nil)
    }
}
